    ...
    <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content text-white modal-inner editstatus-small">
        ...
        <div class="edt-sts-header">
          ...
          <button class="edt-sts-close" id="closeBtn" aria-label="Close" @click="emit('closeStatusModal')">×</button>
        </div>

        <!-- Body -->
        <div class="edt-sts-content">
          <div class="edt-sts-input-row">
            <textarea v-model="statusTxt" ref="statusTxtRef" id="statusInput"
        ...
        <div class="edt-sts-footer">
          <button class="edt-sts-btn" id="cancelBtn" type="button" @click="emit('closeStatusModal')">Cancel</button>
          <button v-if="!is_status_loading" class="edt-sts-btn edt-sts-primary" id="saveBtn" type="button"
            @click="updateStatus()">Save</button>
          <span class="btn-loader" v-if="is_status_loading"></span>
        </div>
      </div>
    </div>
  </div>
...
const emit = defineEmits(['closeStatusModal', 'openEmojiPicker', 'updatedProfileStatus'])
...
const statusTxtRef = ref<HTMLInputElement | null>(null)
const props = defineProps<Props>()
const statusTxt = ref(props.statusTxt)

function addEmoji(emoji: string) {
  const statusInput = statusTxtRef.value
  ...
  if (statusInput) {
    ...
    statusTxt.value = value.substring(0, start) + emoji + value.substring(end);
...
async function updateStatus() {
  ...
  let trim = (statusTxt.value ?? '').trim()
  if (trim.length === 0) {
    showToastError('Please enter status')
    return;
  }
  ...
  if (response.success) {
    emit('updatedProfileStatus', trim)
